...
\begin{enumerate}
    \item \textbf{RMSD mediano de \SI{1,26}{\angstrom} para un dataset diversificado}:
    ...
    El dataset validado (91 prote\'inas, 5.115 pares con mapeo SIFTS expl\'icito) demuestra
...
\begin{enumerate}
    ...
    \item \textbf{Los outliers tienen explicaci\'on biol\'ogica o metodol\'ogica}:
...
\begin{enumerate}
    \item \textbf{Pipeline reproducible con mapeo SIFTS}: Un pipeline completo
...
\begin{enumerate}
    ...
    \item \textbf{Verificar pLDDT pero no confiar exclusivamente en \'el}: El score de
    ...
    \item \textbf{Diversificar datasets de evaluaci\'on}: Seleccionar un PDB
...
\begin{enumerate}
    ...
    an\'alisis con el mismo pipeline permitir\'a una comparaci\'on directa y cuantitativa
    ...
    y reducir\'ia el n\'umero de ``falsos outliers''.
    ...
    \item \textbf{Comparaci\'on con m\'etodos alternativos}: Aplicar el pipeline a
    modelos generados por RoseTTAFold \citep{baek2021accurate}, ESMFold
    \citep{lin2023evolutionary} y OpenFold para establecer un benchmark comparativo
...
diversificaci\'on del dataset para eliminar sesgos, y la cuantificaci\'on bayesiana
