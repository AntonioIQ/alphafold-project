...
\begin{description}
    ...
    cuando el valor real es $\sim$1\,\AA{}. Es el eslab\'on clave del \textit{pipeline}.
...
\noindent donde $\left\| \cdot \right\|$ denota la norma eucl\'idea tridimensional. Expandiendo esta expresi\'on en coordenadas cartesianas:
...
\noindent donde $\bar{\mathbf{r}}^{\text{pred}}$ y $\bar{\mathbf{r}}^{\text{exp}}$ son los centroides de cada conjunto de coordenadas. La rotaci\'on \'optima $\mathbf{R}$ se obtiene a partir de la SVD de $\mathbf{H}$:
...
\noindent donde $d = \text{sign}(\det(\mathbf{V}\mathbf{U}^T))$ asegura una rotaci\'on propia (sin reflexi\'on).
...
El resto de este documento se organiza de la siguiente manera. La Secci\'on~\ref{sec:materiales_metodos} describe las fuentes de datos, el \textit{pipeline} de procesamiento con mapeo SIFTS, y el marco estad\'istico empleado. La Secci\'on~\ref{sec:resultados} presenta los resultados del an\'alisis, incluyendo la estad\'istica descriptiva global, el an\'alisis jer\'arquico en cuatro niveles y la inferencia bayesiana. La Secci\'on~\ref{sec:discusion} discute los hallazgos en el contexto de la literatura, analiza las limitaciones del estudio y sit\'ua los resultados frente a AlphaFold3. Finalmente, la Secci\'on~\ref{sec:conclusiones} resume las contribuciones principales y formula recomendaciones pr\'acticas para la comunidad.
