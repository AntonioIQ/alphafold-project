...
\subsection{\textit{Pipeline} de descarga}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \'Atomos m\'inimos por residuo & 1 & Al menos un \'atomo \textit{backbone} debe estar presente \\
        \'Atomos del \textit{backbone} & N, C$\alpha$, C, O, C$\beta$ & Conjunto de \'atomos utilizados para la superposici\'on \\
...
\begin{table}[H]
    \centering
    \caption{Resumen cuantitativo del \textit{pipeline} de procesamiento de datos.}
...
\begin{itemize}
    ...
    \item \textbf{Raz\'on de momios} (\textit{Odds Ratio}, OR): Compara la probabilidad de obtener una predicci\'on excelente (RMSD $< \SI{1}{\angstrom}$) entre el subgrupo de inter\'es y el resto del \textit{dataset}:
    \begin{equation}
        \text{OR} = \frac{p_{\text{grupo}} / (1 - p_{\text{grupo}})}{p_{\text{resto}} / (1 - p_{\text{resto}})}\,,
        \label{eq:odds_ratio}
    \end{equation}

    \noindent donde $p_{\text{grupo}}$ y $p_{\text{resto}}$ son las proporciones de predicciones excelentes en cada subconjunto.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Longitud de cadena}: variable de control primaria, directamente observable en el \textit{dataset} de 10.432 proteínas.
