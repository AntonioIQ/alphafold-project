...
\section{Detalles de implementaci\'on del \textit{pipeline}}
\label{appendix:pipeline}

\subsection{Arquitectura del \textit{software}}

El \textit{pipeline} se implement\'o como un paquete Python instalable
(\texttt{alphafold-comparison}) con una estructura modular organizada en
cuatro m\'odulos funcionales, como se describe en la Tabla~\ref{tab:modulos}.

\begin{table}[H]
\centering
\caption{M\'odulos del paquete \texttt{alphafold-comparison}.}
\label{tab:modulos}
\begin{tabular}{@{}llp{7cm}@{}}
\toprule
\textbf{M\'odulo} & \textbf{Componente} & \textbf{Funci\'on} \\
\midrule
Configuraci\'on & \texttt{config.py} & Configuraci\'on centralizada mediante variables de entorno \\
Descarga & \texttt{downloader.py} & Descarga automatizada de archivos PDB y modelos AlphaFold \\
Preprocesamiento & \texttt{processor.py} & Mapeo SIFTS de residuos y superposici\'on con algoritmo de Kabsch \\
An\'alisis estructural & \texttt{structural.py} & An\'alisis estad\'istico de RMSD y pruebas de hip\'otesis \\
An\'alisis at\'omico & \texttt{atomic.py} & An\'alisis por elemento qu\'imico (CHONSP) \\
Visualizaci\'on & \texttt{plots.py} & Generaci\'on de figuras y gr\'aficos \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Par\'ametros configurables del \textit{pipeline}.}
\label{tab:params}
\begin{tabular}{@{}llp{7cm}@{}}
\toprule
...
\texttt{MIN\_DIVERSE\_UNIPROTS} & 10.000 & Prote\'inas \'unicas objetivo en \textit{dataset} diverso \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Columnas del archivo SIFTS utilizadas en el \textit{pipeline}.}
...
\textit{Nota: Los datos corresponden al \textit{dataset} validado (91 prote\'inas, 5.115 pares)
con mapeo SIFTS expl\'icito. Las variaciones entre categor\'ias reflejan la composici\'on
del \textit{dataset} m\'as que una tendencia intr\'inseca del algoritmo. V\'ease la
Tabla~\ref{tab:rmsd_tamano} en la secci\'on de Resultados para los datos del
\textit{dataset} diversificado de 10.432 prote\'inas \'unicas.}
